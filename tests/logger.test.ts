...
import { resetLogging, removeLogger } from "../src";
...
describe("Logger edge-cases", () => {
  ...
  it("formatter error doesn't crash", () => {
    ...
    const badHandler: Handler = {
      level: 0,
      formatter: { format() { throw new Error("format failed"); } },
      emit(record) { this.formatter.format(record); },
      close: () => {},
    };
    logger.addHandler(badHandler);
    expect(() => logger.info("test")).not.toThrow();
  ...
  it("captureCallerInfo: false skips stack capture", () => {
    const logger = setupLogging({ name: "no-caller", captureCallerInfo: false });
    logger.info("test");
    const output = writeSpy.mock.calls[0][0] as string;
  ...
  it("captureCallerInfo: true captures real info", () => {
    const logger = setupLogging({ name: "with-caller", captureCallerInfo: true });
    logger.info("test");
    const output = writeSpy.mock.calls[0][0] as string;
...
describe("setupLogging validation", () => {
  ...
  it("throws on invalid level", () => {
    expect(() => setupLogging({ level: "WARN" as any })).toThrow(/Invalid log level/);
  });

  it("throws on invalid fileLevel", () => {
    expect(() => setupLogging({ fileLevel: "BAD" as any })).toThrow(/Invalid fileLevel/);
  });
  ...
  it("accepts valid options without throwing", () => {
    expect(() => setupLogging({ level: "INFO", width: 80, file: null, name: "valid-test" })).not.toThrow();
  });
});
